// Usage: pnpm tsx src/examples/evm/sendManyTransactions.ts

import { config } from "dotenv";
import { serializeTransaction, parseEther, createPublicClient, http, Hex } from "viem";
import { baseSepolia } from "viem/chains";

import { CdpClient } from "../../index.js";
import { APIError } from "../../openapi-client/errors.js";

const NUM_TRANSACTIONS = Number(process.env.NUM_TRANSACTIONS || 10);
...
async function main() {
  ...
  const faucetTxReceipt = await publicClient.waitForTransactionReceipt({ hash: transactionHash });
      ...
      while (true) {
        try {
          ...
          if (
            ...
            console.log(
              `Rate limit exceeded for transaction #${index}, retrying in ${Math.round(delay)}ms (attempt ${retryCount + 1}/${MAX_RETRIES})`,
            );
  ...
  if (successfulTxs.length > 0) {
    ...
    successfulTxs
      ...
      .forEach(({ receipt, index }) => {
        console.log(`Block Number: ${receipt!.blockNumber} - Transaction #${index}`);
      });
  }
}
